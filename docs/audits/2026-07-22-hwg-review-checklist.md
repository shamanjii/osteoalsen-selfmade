...
- [x] 🔴 `sportosteopathie-hamburg-leistung-optimieren.md` — **Größter Fund der Serie:** zwei fiktiv wirkende Patiententestimonials mit Namen/Alter/exakten Erfolgszahlen ("Lisa, 32... Laufsplit um 18 Min. verbessert") — genau die von §11 HWG am striktesten geregelte Kategorie, vom ursprünglichen Scanner wegen abweichendem Format nicht erfasst. Umgeschrieben auf anonymisierte, klar als illustrativ gekennzeichnete Beispiele ohne Erfolgszahlen. Dazu: Crane-2012-Studie (Massage) fälschlich zur Stützung einer 20-30%-Erholungszahl für viszerale Techniken zitiert, mehrere weitere unbelegte Prozentzahlen (8-12% Laufökonomie, 75% Sprunggelenk, 2-5%/10-20% Leistungssteigerung) entschärft.
- [x] ✅ `sportosteopathie-hamburg-marathon-vorbereitung.md` — grün, keine Änderung nötig (gut gehedged, echte passende Quellen: Bernat-Adell 2021, Fairclough 2007)
- [x] 🔧 `sportverletzung-schneller-zurueck-training.md` — **Zahlenreichstes Problem der Serie:** für 5 verschiedene Verletzungstypen wurde durchgängig eine ungefähre Halbierung der Heilungsdauer durch Osteopathie behauptet (z.B. "6-12 Wochen → 3-6 Wochen"), komplett unbelegt — alle 5 entschärft. Plus: erfundene Rückfallrisiko-Aufschlüsselung (60/25/15%), unbelegte 40-50%/20-30%-Zahlen.
- [x] ✅ `vagusnerv-aktivieren-stress-uebungen.md` — mit Abstand am saubersten der ganzen Serie: hat eigenen "Wissenschaftlicher Disclaimer" + "Was ist belegt, was ist Modell"-Abschnitt, der Polyvagal-Theorie explizit als umstritten kennzeichnet. Nur eine Atlas-Vagus-Schwindel-Stelle zur Konsistenz mit anderen Artikeln angepasst.
...
## Nachtrag 2026-07-23: Die drei 🔴-Artikel bearbeitet

- [x] 🔧 `dehnen-rueckenschmerzen-mythos.md` — Disclaimer vor dem Selbsttest ergänzt (keine validierten Instrumente, fachliche Bestätigung vor Verhaltensänderung empfohlen). Direktive Formulierungen ("DEHNEN VERSCHLIMMERT!", "Kein Dehnen mehr!") auf bedingte, professionell begleitete Sprache umgestellt.
- [x] 🔧 `spannungskopfschmerzen-muskel-oder-hws.md` — gleicher Fix: Disclaimer vor "3-Phasen-Differenzierungs-Protokoll", "Phase 1: Selbstdiagnose" → "Erste Orientierung", Testergebnis-Interpretationen entschärft.
- [x] 🔧 `rueckenschmerzen-osteopathie-ganzheitliche-behandlung-hamburg.md` — letzte unbelegte "Wissenschaftlich belegt"-Aussage entschärft, irreführendes `sourceCount: 31`-Feld (0 tatsächliche Quellen) aus Frontmatter entfernt.
